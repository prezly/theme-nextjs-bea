...
`instrumentation.ts` exports Next.js's `onRequestError` hook, which fires for every unhandled server error during rendering, route handlers and middleware. Each one is logged as a JSON line with `event: "render_error"`, the request path and method, whether the request was a Flight (`rsc`) navigation or a `prefetch`, the route, the render source and the error digest. Next.js commits a Flight response's status before rendering finishes, so a render error on a navigation reaches the browser as an HTTP 200 with the error inside the body; this count is what decides whether caching navigations in Varnish is worth a body-inspecting layer (DEV-24195). Prefetches render nothing and are cached without one (DEV-24166).
